namespace TornadoTest2.Controllers
{
    ...
    // - Продюсер вызывает OnDataReceived из контекста прерывания/callback (должно быть быстро). Используется кольцевой буфер, чтобы не тратить время на выделение памяти
